...
async def generate_response(request):
    ...
    try:
        ...
        if base_url == "":
            ...
            response = await get_echo_response(client_request)
        elif base_url == "files":
            response = await get_files_response(client_request)
        elif base_url == "user-agent":
            response = await get_user_agent_response(client_request)
...
async def get_echo_response(client_request):
    request_target = client_request[0].split()[1]
...
async def get_user_agent_response(client_request):
    request_headers = {}
